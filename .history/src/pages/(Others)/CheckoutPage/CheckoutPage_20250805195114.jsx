...
const CheckoutPage = () => {
  ...
  useEffect(() => {
    const fetchCart = async () => {
      try {
        const res = await privateApiClient.get("/api/v1/cart");
  ...
  const handleCheckout = async () => {
    ...
    try {
      const res = await privateApiClient.post("/api/v1/orders", {
        paymentMethod,
        notes: note,
        deliveryInfo: {
        ...
        <div className="w-full md:w-1/2 p-8 bg-[#111827] flex flex-col justify-between">
          ...
          <div className="mt-6 border-t border-gray-700 pt-4 text-lg font-semibold">
            ...
            <button
              ...
              className="w-full bg-green-600 hover:bg-green-700 text-white py-3 rounded-xl font-medium shadow-md"
            >
              Place Order
            </button>
          </div>
